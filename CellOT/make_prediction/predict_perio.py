...
def predict_from_unstim_data(result_path, unstim_data_path, output_path):
    ...
    predicted=predicted[predicted.obs['drug']=='IFNa']
...
for cell_type in perio_cell_list_:
    for stim in perio_stim_list_:
        if stim=='P._gingivalis':
            doms_stim='LPS'
            unstim_data_path = f'./datasets/doms_concatenated/doms_concatenated_{doms_stim}_{cell_type}.h5ad'
        else:
            unstim_data_path = f'./datasets/doms_concatenated/doms_concatenated_{stim}_{cell_type}.h5ad'
        result_path = f"./results/perio_training_LR2/{stim}_{cell_type}/model-cellot"
        output_path = f'./results/perio_training_LR2/{stim}_{cell_type}/model-cellot/pred_doms_uncorrected.h5ad'
        ada = predict_from_unstim_data(result_path, unstim_data_path, output_path)
        print(f"Predicted {cell_type} for {stim} 2")