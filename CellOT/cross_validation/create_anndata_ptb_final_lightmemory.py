...
OUTPUT_DIR = "/home/groups/gbrice/ptb-drugscreen/ot/cellot/datasets/ptb_concatenated_per_conditon_celltype" # Changed output dir name
...
def process_fcs_file_simplified(file_path, patient, drug, stim, cell_type):
    ...
    obs = pd.DataFrame({
        ...
        "drug": [drug if not drug.startswith("DMSO") else "DMSO"] * data_transformed.shape[0],
...
def main():
    ...
    for cell_type_file, condition in zip(EXPECTED_CELL_TYPES, ['HV', 'PV']):

        combined_final = None
        c=0
        for ad_path in all_files:
            if cell_type_file in ad_path and condition in ad_path:
                file_path = os.path.join(RAW_DIR, ad_path)
                patient, drug, stim, cell_type = parse_filename(ad_path)
                if cell_type_file!=cell_type:
                    print('not the good cell name')
                    continue
                if patient[:2]!=condition:
                    print('not the good patient name')
                    continue
                if patient not in EXPECTED_PATIENTS or drug not in EXPECTED_DRUGS or stim not in EXPECTED_STIMS or cell_type not in EXPECTED_CELL_TYPES:
                    print(f"Warning: Skipping file {ad_path} due to unexpected factor(s).")
                    continue
                ad=process_fcs_file_simplified(file_path, patient, drug, stim, cell_type)
                if combined_final is None:
                    combined_final = ad
                else:
                    combined_final = ad.concat([combined_final, ad], join='outer', axis=0, label=f"{cell_type}_{condition}")
                c+=1
        out_filename = f"{cell_type}_{condition}.h5ad"
        if condition=='PV' and c!=11*96:
            print(f'not the good number of files for {cell_type_patient} and {condition}')
        if condition=='HV' and c!=8*96:
            print(f'not the good number of files for {cell_type_patient} and {condition}')
