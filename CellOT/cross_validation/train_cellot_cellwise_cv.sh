...
#SBATCH --output=logs/ptb_different_IO_batchcorr/train_cellot_cv_%A_%a.out # Changed log name
#SBATCH --error=logs/ptb_different_IO_batchcorr/train_cellot_cv_%A_%a.err # Changed log name
...
model='different_IO' # choose among 'olivier', 'peter', 'original','all_markers' 
...
PATIENT_FILE="${BASE_DIR}/patients_HV.txt" #<-- Path to patient list, here only HV
